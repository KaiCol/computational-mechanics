...
x = sum (r[:,0])/len(r[:,0])
y = sum (r[:, 1])/len(r[:,1])
print(f'The average location of the particle is {sum (r[:,0])/len(r[:,0])} , {sum (r[:, 1])/len(r[:,1])}\
\nthe standard deviation of the particle is:\
\n{np.std(r[:,0],ddof=1)} in the x\
\n{np.std(r[:,1],ddof=1)} in the y')
```
...
from numpy.random import default_rng
rng = default_rng()
#Part A - Generating 100 random x and theta
x = rng.random(100)
theta = rng.random(100) * 2 * np.pi
#Part B Calculating the 100 needle ends
xendp, xendm = [x - np.cos(theta), x + np.cos(theta)]
#Part C - Logical 
test = np.logical_and(np.max([xendp, xendm], axis = 0) > 0,
                      np.min([xendp, xendm], axis = 0) < 0)

print (f'The actual value of 2/pi is {round(2/np.pi,3)}, the value calculated from the needle problem is {round(sum(test)/100,3)}')
```

```{code-cell} ipython3
#Number 2 with forloop
import numpy as np
from numpy.random import default_rng
rng = default_rng()
#Creating particles and steps
N_steps = 10
num_particles = 100
r_final = np.zeros((num_particles))
for i in range(0, num_particles):
    dx = rng.random(N_steps) - 0.5
    dy = rng.random(N_steps) - 0.5
    r = np.sqrt(np.sum(dx)**2+np.sum(dy)**2)#Summing them into total distance
    r_final[i] = r
#Logic
result = sum(np.logical_not(r_final < 0.5))
print(f'{result} is how many particles traveled more than 0.5m from the origin')
```

```{code-cell} ipython3
#Number 2 W/o forloop
import numpy as np
from numpy.random import default_rng
rng = default_rng()
#Creating the particles and their steps
N_steps = 10
N_particles = 100
dx = rng.random((N_steps, N_particles)) - 0.5
dy = rng.random((N_steps, N_particles)) - 0.5
#Summing and calculating distance
x = np.sum(dx, axis = 0)
y = np.sum(dy, axis = 0)
r_final = np.sqrt(x**2 + y**2)
#logic to calculate r has moved more than 0.5
result = sum(np.logical_not(r_final < 0.5))
print(f'{result} is how many particles traveled more than 0.5m from the origin')
```
...
def montecarlo_buckle(E,r_mean,r_std,L,N=100):
    ...
    total_data: the total data set of the 100 rods
    '''
    r = rng.random(N)
    rmax = r_mean + r_std
    rmin = r_mean - r_std
    rrand = rmin+(rmax-rmin)*r
    
    top = np.pi**3 * E * rrand**4
    bottom = 16 * L**2
    total_data = top/bottom
    
    mean_buckle_load= np.mean(total_data)
    std_buckle_load = np.std(total_data)
    
    
    return mean_buckle_load, std_buckle_load , total_data
```
...
from numpy.random import default_rng
rng = default_rng()
buckle_load, buckle_std, all_data = montecarlo_buckle(200e9,0.01,0.001,5)
print (f'The mean buckle load is {round(buckle_load,2)} N and the std is {round(buckle_std,2)} N')
#print(200e9)
#print(all_data)
```
...
#Part B
L = 0.7555
buckle_load, buckle_std, all_data = montecarlo_buckle(200e9,0.01,0.001,L)
z = sum(np.logical_not(all_data < (1000*9.81)))
#print(all_data)
print(f'With a L of {L}, {z}% of the rods will fail')
```
