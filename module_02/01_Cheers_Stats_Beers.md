...
```{code-cell} ipython3
print('abv', len(abv)/len(abv_series.values))
print('ibu', len(ibu)/len(ibu_series.values))
```
...
#Plotting
pyplot.hist(ibu, bins=20, color=(0.9, 0.5, 0, 0.9), histtype='bar', edgecolor='k') 
...
```{code-cell} ipython3
'''I think that skewness will cause the percentage distributions to lean one way or another depending on the type of skew.
It will also change the location of the medium '''
x[x<mean_x]/len(x)*100
```
...
```{code-cell} ipython3
trans = cpus['MOS transistor count']
trans_clean = trans.dropna()
print(f"{round(100*(1-(len(trans_clean)/len(trans))),2)} percent is Nan and there are \
{len(trans)-len(trans_clean)} NaNs in transistors")
```
