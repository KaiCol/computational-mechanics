...
plt.plot(t,x0*np.cos(w*t), label='analytical')
plt.ylim(-8,8)
plt.legend();
...
```{code-cell} ipython3
'''The implicit method is kind of like a self correcting method, it calculates then goes back and reiterates that same step
this acts like a correctional step that would solve the increasing magnitudes.'''
```
...
```{code-cell} ipython3
#Setting Up Huen's Method same as before
w = 2
period = 2*np.pi/w
dt = period/10  # time intervals per period 
T = 8*period   # simulation time, in number of periods
...
plt.xlabel('$\Delta t$')
plt.ylabel('Error')
plt.title('Convergence of Huen Method');
```
...
for N in [100,400,700,1000]:
    ...
    statee = state.copy()
    staterk2 = state.copy()
    stateheun = state.copy()
    for i in range(1, N):
        statee[i] = eulerstep(statee[i-1], smd, dt)
        staterk2[i] = rk2_step(staterk2[i-1], smd, dt)
        stateheun[i] = heun_step(stateheun[i-1], smd, dt)
    plt.figure(f"n = {N}")
    plt.plot(t, statee[:,0], label = f'= Euler')
    plt.plot(t, staterk2[:,0], label = f'= rk2')
    plt.plot(t, stateheun[:,0],'--' , label = f'heun')
    plt.title(f'n = {N}')
    plt.ylim((-3,3))
    plt.xlim((0,10))
    plt.legend(bbox_to_anchor=(1.05,1.0), loc = "upper left")
#for i in ['euler', 'rk2', 'heun']:
#    plt.figure(i)
#    plt.title(f'{i}\'s method')
#    plt.ylim((-3,3))
#    plt.xlim((0,10))
#    plt.legend(bbox_to_anchor=(1.05,1.0), loc = "upper left")
```
...
```{code-cell} ipython3
'''From the looks of it, Heun and rk2 method has very little amount of time steps needed, sub 200 as 100 is too low
 euler on the other hand takes a lot of steps just to converge, and even then, there are little errors.'''
```
