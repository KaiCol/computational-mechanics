...
```{code-cell} ipython3
plt.boxplot(dprice, labels=['Change in Apple Opening Prices']);
Q1 = np.percentile(dprice, q=25)
Q2 = np.percentile(dprice, q=50)
Q3 = np.percentile(dprice, q=75)
dprice_max = max(dprice) 
dprice_min = min(dprice)

print('The first quartile for abv is {}'.format(Q1))
print('The second quartile for abv is {}'.format(Q2))
print('The third quartile for abv is {}'.format(Q3))
print(f'The max change in opening price is {dprice_max} and min is {dprice_min}')
```
